...
private tuple[str,int] generate_exp_p(&T e, int count) {
    ...
    switch (e) {
        ...
        // This isn't really &T...
        // TODO: would be nice to combine these two...
        case &T n(*&T _): {
...
private tuple[str,int] synthesise_p(Fancy::AST::Exp exp, int count) {
    ...
    switch (exp) {
        ...
        case &T n(&T e0, &T e1): {
            // println("e0: <e0> typeOf(e0): <typeOf(e1)>\ne1: <e1> typeOf(e1): <typeOf(e1)>\n");
...
public str synthesise(Fancy::AST::Exp fp, Core::AST::Exp cp) {
...
// translator...somewhow
public str synthesise(loc floc, loc cloc) {
    // println("BEGIN: synth");
    fp = Fancy::Load::load(floc);
    // println("\n\nFP: <fp>\n\n");
    cp = Core::Load::load(cloc);
    return synthesise(fp.exp, cp.exp);
}
